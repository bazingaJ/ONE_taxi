...
+(void)securityTextWithPass:(NSString *)password success:(void (^)(NSString *))success failure:(void (^)(NSError *))failure
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [GetData getDataWithUrl:@"http://114.55.57.237/tad/client/getPublicKey.json" params:nil success:^(id response) {
            
            dispatch_async(dispatch_get_main_queue(), ^{
                NSString *str1 =  [response objectForKey:@"publicKey"]; // 公钥
                NSString *security = [RSA encryptString:password publicKey:str1];
                [[NSUserDefaults standardUserDefaults]setObject:security forKey:@"securityPassWord"];
                if (str1) {
                    if (success) {
                        success(security);
                    }
