...
-(void)getData
{
    ...
    // 请求单子列表
    [JDCallCarTool getCallCarInfoWithType:@"0" inVC:self orderId:nil Success:^(NSMutableArray *modelArr, int status) {
...
-(void)setUpOrderCount
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [JDCallCarTool getCallCarListWithType:@"2" inVC:self Success:^(NSMutableArray *modelArr, int orderFinishCount) {
            
            self.myOrderCount = modelArr.count;
            orderFinishCount = [[[NSUserDefaults standardUserDefaults]objectForKey:@"orderFinishCount"] intValue];
            dispatch_async(dispatch_get_main_queue(), ^{
                //JDLog(@"%d",orderId);
                // 今日已成功接单数，设置label数据
                self.todayOrder.text = [NSString stringWithFormat:@"今日已成功接单：%d",orderFinishCount];
...
-(void)setUpNavgationBar
{
    ...
    [self addRightBtnWithTitle:@"接单历史" action:@selector(clickRightItem)];
...
-(void)clickAlertViewSureBtn:(JDCallCarAlertViewFrame *)callCarAlertFrame
{
    ...
    if (self.myOrderCount<2) { // 还可以预约
        ...
        [JDCallCarTool getCallCarInfoWithType:@"1" inVC:self orderId:data.orderId Success:^(NSMutableArray *modelArr, int status) {
            JDLog(@"%d",status);
            if (status==0) { // 接单成功
