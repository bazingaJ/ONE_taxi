...
- (void)viewDidLoad {
    [super viewDidLoad];
    [MyTool addMBProgressWithView:self.view style:0];
    [MyTool showMBWithTitle:@"正在加载~~~~"];
    _moneys = 0;
    
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        
        [JDPeccancyHttpTool peccancyGetDataInVc:self Success:^{
            [MyTool hiddenMBWithDelayTimeInterval:1];
...
-(void)chooseTableViewOrNoMessView:(NSInteger)count
{
    if (count==0) {
        ...
        JDNoMessageView *noMessView = [[JDNoMessageView alloc] initWithFrame:CGRectMake(0, 120, JDScreenSize.width, self.tableView.bounds.size.height)];
        noMessView.message = @"当前没有违章记录";
        [self.view addSubview:noMessView];
