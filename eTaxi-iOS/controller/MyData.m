...
- (void)getChangeNewPhoneWithPhoneNo:(NSString *)phone WithNewPhoneNo:(NSString *)nPhone WithType:(NSString *)type validCode:(NSString *)validCode WithCompletionBlock:(void (^)(NSString *, NSString *))block
{
    ...
    NSDictionary * inDic =[NSDictionary dictionaryWithObjectsAndKeys:phone,@"phoneNo",nPhone,@"newPhoneNo",[[NSUserDefaults standardUserDefaults] objectForKey:@"securityPassWord"],@"password",type,@"type",LOGINTIME,@"loginTime",validCode,@"validCode", nil];
    AFHTTPRequestOperationManager * manager =[AFHTTPRequestOperationManager manager];
    manager.responseSerializer=[AFJSONResponseSerializer serializer];
    manager.responseSerializer.acceptableContentTypes=[NSSet setWithObjects:@"text/html",@"application/json", nil];
    manager.requestSerializer.timeoutInterval=10;
