...
- (void)GeTuiSdkDidReceivePayload:(NSString *)payloadId andTaskId:(NSString *)taskId andMessageId:(NSString *)aMsgId andOffLine:(BOOL)offLine fromApplication:(NSString *)appId
{
    ...
    else if(  [methodName isEqualToString:@"order"]
            ||[methodName isEqualToString:@"appointCall"]
            ||[methodName isEqualToString:@"use_callCar"])
    { // 接单通知（取消订单，完成订单）
            ...
            { // 真机
                ...
                { // 召车语音开关打开
                    ...
                    if ([self.window.rootViewController isKindOfClass:[UINavigationController class]]) {
                        ...
                        if ([[nav.viewControllers lastObject] isKindOfClass:[JDCallCarListViewController class]]) {
                            [[NSNotificationCenter defaultCenter]postNotificationName:@"取消订单" object:nil];
                        }else{
                            [GetData addAlertViewInView:self.window.rootViewController title:@"温馨提示:" message:@"您有一条订单已取消！" count:0 doWhat:^{
