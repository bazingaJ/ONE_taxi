...
+(instancetype)shareInstance
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (!x||!y) {
            x=Width-size.width-5, y=Height-size.height-45;
...
-(void)setUpNumber
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [JDCallCarTool getCallCarInfoWithType:@"0" inVC:nil orderId:nil Success:^(NSMutableArray *modelArr, int status) {
...
//                self.x = 5;
                ...
                self.y = Height-self.bounds.size.height-45;
            }];
